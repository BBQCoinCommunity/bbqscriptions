...
const bbqcore = require('bitcore-lib-bbqcoin')
const axios = require('axios')
const fs = require('fs')
const dotenv = require('dotenv')
const mime = require('mime-types')
const express = require('express')
const { PrivateKey, Address, Transaction, Script, Opcode } = bbqcore
const { Hash, Signature } = bbqcore.crypto

dotenv.config()

if (process.env.TESTNET == 'true') {
    bbqcore.Networks.defaultNetwork = bbqcore.Networks.testnet
}
...
async function bqc20() {
  let subcmd = process.argv[3]

  if (subcmd === 'mint') {
    await bqc20Transfer("mint")
...
async function bqc20Deploy() {
  ...
  const parsedBQC20Tx = JSON.stringify(bqc20Tx);

  // encode the bqc20Tx as hex string
  const encodedBQC20Tx = Buffer.from(parsedBQC20Tx).toString('hex');

  console.log("Deploying bqc-20 token...");
  await mint(argAddress, "text/plain;charset=utf-8", encodedBQC20Tx);
}

async function bqc20Transfer(op = "transfer") {
  const argAddress = process.argv[4]
  const argTicker = process.argv[5]
  ...
  for (let i = 0; i < argRepeat; i++) {
    console.log("Minting bqc-20 token...", i + 1, "of", argRepeat, "times");
    await mint(argAddress, "text/plain;charset=utf-8", encodedBQC20Tx);
  }
}
...
async function walletSync() {
    ...
    console.log('Syncing UTXOs with local BBQcoin node via RPC')
    ...
    // Fetch UTXOs from the BBQcoin node
